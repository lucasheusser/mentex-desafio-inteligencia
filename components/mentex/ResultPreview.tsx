...
export function ResultPreview({ sessionId, preview, onUnlock, onRestart }: {
  sessionId: string;
  preview: Preview;
  onUnlock: (accessToken: string) => Promise<void>;
  onRestart: () => void;
}) {
  ...
  const checkout = async (outcome: 'approved' | 'pending' | 'declined' | 'cancelled') => {
    ...
    try {
      const response = await fetch(`/api/session/${sessionId}/checkout`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ outcome }) });
      const data = await response.json() as { status?: PaymentStatus; accessToken?: string; error?: string };
      ...
      if (data.status === 'approved' && data.accessToken) {
        ...
        await onUnlock(data.accessToken);
      }
    } catch (caught) {
      setStatus('error');
    ...
    <section className="result-preview-shell" aria-labelledby="result-ready">
      ...
      <div className="preview-grid">
        ...
        <aside className="paywall-card">
          <div className="demo-flag">DEMONSTRAÇÃO · NENHUMA COBRANÇA REAL</div>
          ...
          <Button className="unlock-button" disabled={status === 'loading'} onClick={() => checkout('approved')}>{status === 'loading' ? 'Processando demonstração…' : 'Liberar resultado no modo demo'} <ArrowRight /></Button>
          <p className="security-copy"><ShieldCheck /> Demonstração segura. Nenhuma assinatura ou cobrança será criada.</p>
          <div className="demo-states"><span>Testar outros estados:</span><button onClick={() => checkout('pending')}>Pendente</button><button onClick={() => checkout('declined')}>Recusado</button><button onClick={() => checkout('cancelled')}>Cancelado</button></div>
